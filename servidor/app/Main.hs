...
-- Convierte el tablero en formato JSON
...
bestMv :: Element -> Board -> Int -> Maybe ([Integer], Int)
bestMv player pos level
  | null tree = Nothing
  | otherwise = Just $ minimax (transTree (last tree))
  where
    tree = genTree player pos level
...
--        let bm=head ls 
--            newpos1 = newPos X bm pos --GENERA LA NUEVA POSICION DEL TABLERO
--        mMove X bm pos --IMPRIME LO DE LA IA
--       putStr $ show bm
--        putStr $ "\n"
...
--        putStr (show square)
--        let 
--          newpos2 = newPos O square newpos1 --NUEVO TABLERO
--        mMove O square newpos1  --IMPRIME
...
main :: IO ()
main = do
    boardRef2 <- newIORef posVariant -- Estado inicial del tablero
    turnRef2 <- newIORef X      -- Turno inicial
    
    boardRef <- newIORef posIni -- Estado inicial del tablero
    turnRef <- newIORef X       -- Turno inicial
    
    scotty 3000 $ do
        middleware simpleCors  -- Middleware que habilita CORS

        -- Endpoint para obtener el estado actual del tablero
        get "/board" $ do
            board <- liftIO $ readIORef boardRef
            turn <- liftIO $ readIORef turnRef
            json $ object ["tablero" .= board, "turno" .= show turn]

        get "/reset" $ do
            liftIO $ writeIORef boardRef posIni -- Reinicia el tablero al estado inicial
            text "Tablero Iniciado"
        
        get "/reset2" $ do
            liftIO $ writeIORef boardRef posVariant -- Reinicia el tablero al estado inicial
            text "Tablero Iniciado"

        get "/movementIA" $ do
            board <- liftIO $ readIORef boardRef
            case bestMv X board 4 of
              Nothing -> do
                  -- Si no hay movimientos, devuelve un JSON indicando el estado
                  liftIO $ putStrLn "Error: No hay movimientos válidos para la IA"
                  json $ object ["error" .= ("No hay movimientos válidos para la IA" :: String)]
              Just (moves, _) -> do
                -- Si hay movimientos válidos
                case tail moves of
                  [] -> do
                      liftIO $ putStrLn "Error: Lista de movimientos está vacía después de aplicar tail"
                      json $ object ["error" .= ("No se encontraron movimientos válidos tras aplicar tail" :: String)]
                  (bm:_) -> do
                      let newpos1 = newPos X bm board
                      liftIO $ mMove X bm board
                      liftIO $ writeIORef boardRef newpos1
                      liftIO $ putStrLn $ "Movimiento realizado: " ++ show bm
                      json $ object ["fichas_negras" .= show (count X newpos1), "fichas_blancas" .= show (count O newpos1), "movimientos" .= show (allNum O newpos1), "num_movimientos" .= show (length (allNum O newpos1))]
                      --text "GOOD IA"

        get "/movementPlayerVsIA/:position" $ do
            beamStr <- pathParam "position"
            let beam = read beamStr :: Integer

            board <- liftIO $ readIORef boardRef
            let newpos2 = newPos O beam board --NUEVO TABLERO
            liftIO $ writeIORef boardRef newpos2 -- Actualiza el estado del tablero

            -- Devuelve el nuevo tablero
            json $ object ["tablero" .= newpos2]

        -- Endpoint para obtener el estado actual del tablero
        get "/boardVariant" $ do
            board <- liftIO $ readIORef boardRef2
            turn <- liftIO $ readIORef turnRef2
            json $ object ["tablero" .= board, "turno" .= show turn]
